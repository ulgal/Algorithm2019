...
class Solution2 {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int test_case = 1; test_case <= 10; test_case++) {
			 ...
			 */
			/////////////////////////////////////////////////////////////////////////////////////////////
			Answer[0] = BucketA[n - 1][0]; // a 의 갯수
			Answer[1] = BucketB[n - 1][0]; // b 의 갯수
			Answer[2] = BucketC[n - 1][0]; // c 의 갯수
	...
	private static void calculate() {
		...
		for (i = 0; i < n; ++i) {
			...
			case 'a':
				BucketA[0][i] = 1;
				BucketB[0][i] = BucketC[0][i] = 0;
				break;
			case 'b':
				BucketB[0][i] = 1;
				BucketA[0][i] = BucketC[0][i] = 0;
				break;
			case 'c':
				BucketC[0][i] = 1;
				BucketA[0][i] = BucketB[0][i] = 0;
				break;
			}
		}
		for (i = 1; i < n; ++i) {
			for (j = 0; j < n - i; ++j) {
				...
				for (k = 0; k < i; ++k) {
					BucketA[i][j] += (BucketA[k][j] * BucketC[i - k - 1][j + k + 1])
							+ (BucketB[k][j] * BucketC[i - k - 1][j + k + 1]) + (BucketC[k][j] * BucketA[i - k - 1][j + k + 1]);
					BucketB[i][j] += (BucketA[k][j] * BucketA[i - k - 1][j + k + 1])
							+ (BucketA[k][j] * BucketB[i - k - 1][j + k + 1]) + (BucketB[k][j] * BucketB[i - k - 1][j + k + 1]);
					BucketC[i][j] += (BucketB[k][j] * BucketA[i - k - 1][j + k + 1])
							+ (BucketC[k][j] * BucketB[i - k - 1][j + k + 1]) + (BucketC[k][j] * BucketC[i - k - 1][j + k + 1]);
				}
			}
		}
	}
